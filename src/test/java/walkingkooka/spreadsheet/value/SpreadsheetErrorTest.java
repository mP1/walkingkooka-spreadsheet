...
import walkingkooka.terminal.HasTerminalErrorTextTesting2;
...
public final class SpreadsheetErrorTest implements ParseStringTesting<SpreadsheetError>,
    ...
    HasTerminalErrorTextTesting2<SpreadsheetError>,
    HasValueTesting {
    ...
    @Test
    public void testIsFormatterWithDiv0() {
        this.isFormatterAndCheck(
            SpreadsheetErrorKind.DIV0.spreadsheetError(),
            false
        );
    }
    ...
    @Test
    public void testIsFormatterWithFormatterError() {
        this.isFormatterAndCheck(
            SpreadsheetErrorKind.FORMATTING.spreadsheetError(),
            true
        );
    }

    @Test
    public void testIsFormatterWithValidationError() {
        this.isFormatterAndCheck(
            SpreadsheetErrorKind.VALIDATION.spreadsheetError(),
            false
        );
    ...
    private void isFormatterAndCheck(final SpreadsheetError error,
                                     final boolean expected) {
        this.checkEquals(
            expected,
            error.isFormatter(),
    ...
    @Test
    public void testSetKindWithNullFails() {
        assertThrows(
            NullPointerException.class,
    ...
    @Test
    public void testSetKindWithSame() {
        final SpreadsheetError error = SpreadsheetError.with(KIND, MESSAGE, VALUE);
        assertSame(
            error,
            error.setKind(KIND)
    ...
    @Test
    public void testSetKindWithDifferent() {
        final SpreadsheetError error = SpreadsheetError.with(KIND, MESSAGE, VALUE);

        final SpreadsheetErrorKind differentKind = SpreadsheetErrorKind.ERROR;
        ...
        final SpreadsheetError different = error.setKind(differentKind);

        this.kindAndCheck(different, differentKind);
    ...
    @Override
    public SpreadsheetError createHasTerminalErrorText() {
        return this.createObject();
    }
