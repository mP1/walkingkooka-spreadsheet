...
public final class CellReferenceSpreadsheetParserTokenTest extends ParentSpreadsheetParserTokenTestCase<CellReferenceSpreadsheetParserToken>
    ...
    public void testWith() {
        final ColumnReferenceSpreadsheetParserToken column = this.column();
        final RowReferenceSpreadsheetParserToken row = this.row();
        final String text = ROW_TEXT + ":" + COLUMN_TEXT;
        final CellReferenceSpreadsheetParserToken cell = this.createToken(text, row, column);
    ...
    @Override
    CellReferenceSpreadsheetParserToken createToken(final String text, final List<ParserToken> tokens) {
        return SpreadsheetParserToken.cellReference(tokens, text);
    }

    private ColumnReferenceSpreadsheetParserToken column() {
        return column(COLUMN_VALUE);
    }

    private ColumnReferenceSpreadsheetParserToken column(final int value) {
    ...
    private RowReferenceSpreadsheetParserToken row() {
        return row(ROW_VALUE, ROW_TEXT);
    }

    private RowReferenceSpreadsheetParserToken row(final int value, final String text) {
    ...
    private void checkCell(final CellReferenceSpreadsheetParserToken cell,
                           final RowReferenceSpreadsheetParserToken row,
                           final ColumnReferenceSpreadsheetParserToken column) {
        this.checkEquals(
    ...
    @Override
    public CellReferenceSpreadsheetParserToken createDifferentToken() {
        return this.createToken("D9", Lists.of(this.column(9), this.row(3, "D")));
    }

    @Override
    public Class<CellReferenceSpreadsheetParserToken> type() {
        return CellReferenceSpreadsheetParserToken.class;
    }

    @Override
    public CellReferenceSpreadsheetParserToken unmarshall(final JsonNode from,
    ...
    public void testReference() {
        final CellReferenceSpreadsheetParserToken token = this.createToken();
