 ...
 */
enum NumberSpreadsheetTextFormatterNormalOrScientific {
    ...
    NORMAL {
        @Override
        NumberSpreadsheetTextFormatterComponentContext context(final BigDecimal value,
            ...
            return NumberSpreadsheetTextFormatterComponentContext.with(
                    NumberSpreadsheetTextFormatterDigits.integer(NumberSpreadsheetTextFormatterMinusSign.fromSignum(valueSign), integerDigits, formatter.thousandsSeparator),
                    NumberSpreadsheetTextFormatterDigits.fraction(fractionDigits),
                    NO_EXPONENT,
                    formatter,
                    context);
        }
    ...
    SCENTIFIC {
        ...
        NumberSpreadsheetTextFormatterComponentContext context(final BigDecimal value,
            ...
            return NumberSpreadsheetTextFormatterComponentContext.with(
                    NumberSpreadsheetTextFormatterDigits.integer(NumberSpreadsheetTextFormatterMinusSign.fromSignum(value.signum()), digits.substring(0, integerDigitCount), formatter.thousandsSeparator),
                    NumberSpreadsheetTextFormatterDigits.fraction(digits.substring(integerDigitCount, Math.min(integerDigitCount + fractionDigitCount, digitCount))),
                    NumberSpreadsheetTextFormatterDigits.exponent(NumberSpreadsheetTextFormatterMinusSign.fromSignum(exponent), String.valueOf(Math.abs(exponent))),
                    formatter,
                    context);
        }
    };
    ...
    /**
     * Creates a new {@link NumberSpreadsheetTextFormatterComponentContext} which will accompany the current
     * format request. Note context cannot be recycled as they contain state.
     */
    abstract NumberSpreadsheetTextFormatterComponentContext context(
            final BigDecimal value,
            final NumberSpreadsheetTextFormatter formatter,
            final SpreadsheetTextFormatContext context);

    private final static NumberSpreadsheetTextFormatterDigits NO_EXPONENT = NumberSpreadsheetTextFormatterDigits.exponent(NumberSpreadsheetTextFormatterMinusSign.NOT_REQUIRED, "");
}
