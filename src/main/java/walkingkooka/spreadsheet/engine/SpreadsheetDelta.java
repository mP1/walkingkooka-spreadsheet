...
public abstract class SpreadsheetDelta implements Patchable<SpreadsheetDelta>,
    ...
    private SpreadsheetLabelNameSet filterDeletedLabels(final Set<SpreadsheetLabelName> deletedLabels) {
        return SpreadsheetLabelNameSet.EMPTY.setElements(deletedLabels);
    }
     ...
     * Creates a {@link JsonNode patch} which may be used to {@link #patchCells(SpreadsheetCellReferenceOrRange, JsonNode, JsonNodeUnmarshallContext)}.
     */
    public static JsonNode cellsValueTypePatch(final Map<SpreadsheetCellReference, Optional<ValueType>> cellToValueTypes,
                                               final JsonNodeMarshallContext context) {
        ...
        return cellsPatchFromMap(
            cellToValueTypes,
            FORMULA_PROPERTY,
            (Optional<ValueType> t) -> SpreadsheetFormula.valueTypePatch(t, context)
        );
    }
     ...
     * Creates a {@link Currency} which can then be used to as an argument to {@link #patchCells(SpreadsheetCellReferenceOrRange, JsonNode, JsonNodeUnmarshallContext).}
     */
    public static JsonNode currencyExchangeRaterPatch(final Optional<CurrencyExchangeRaterSelector> currencyExchangeRaterSelector,
                                                      final JsonNodeMarshallContext context) {
     ...
     * Creates a {@link JsonNode} patch that may be used by {@link #patch(JsonNode, JsonNodeUnmarshallContext)}.
     */
    public static JsonNode valueTypePatch(final Optional<ValueType> valueType,
                                          final JsonNodeMarshallContext context) {
    ...
    private static JsonObject resolveCellLabelsCell(final JsonObject cells,
        ...
        for (final Map.Entry<JsonPropertyName, JsonNode> child : cells.objectOrFail().asMap().entrySet()) {
            final SpreadsheetValidationReference cellOrLabelName = SpreadsheetSelection.parseValidationReference(
                child.getKey()
                    .value()
            );
