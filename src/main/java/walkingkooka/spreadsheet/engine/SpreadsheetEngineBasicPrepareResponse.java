 ...
 */
final class SpreadsheetEngineBasicPrepareResponse {

    static SpreadsheetDelta prepare(final SpreadsheetEngineBasic engine,
                                    final SpreadsheetEngineBasicChanges changes,
                                    final SpreadsheetViewportWindows window,
                                    final SpreadsheetEngineContext context) {
        return new SpreadsheetEngineBasicPrepareResponse(
            engine,
            changes,
            window,
            context
    ...
    private SpreadsheetEngineBasicPrepareResponse(final SpreadsheetEngineBasic engine,
                                                  final SpreadsheetEngineBasicChanges changes,
                                                  final SpreadsheetViewportWindows window,
                                                  final SpreadsheetEngineContext context) {
        this.engine = engine;
    ...
    private void columns() {
        if (this.shouldSaveUpdateColumns || this.shouldDeleteColumns) {
            ...
            for (final SpreadsheetEngineBasicChangesCache<SpreadsheetColumnReference, SpreadsheetColumn> referenceAndColumn : this.changes.columns.values()) {
                final SpreadsheetColumnReference column = referenceAndColumn.reference;

                final SpreadsheetEngineBasicChangesCacheStatusColumn status = (SpreadsheetEngineBasicChangesCacheStatusColumn) referenceAndColumn.status();
    ...
    private void rows() {
        if (this.shouldSaveUpdateRows || this.shouldDeleteRows) {
            ...
            for (final SpreadsheetEngineBasicChangesCache<SpreadsheetRowReference, SpreadsheetRow> referenceAndRow : this.changes.rows.values()) {
                final SpreadsheetRowReference row = referenceAndRow.reference;

                final SpreadsheetEngineBasicChangesCacheStatusRow status = (SpreadsheetEngineBasicChangesCacheStatusRow) referenceAndRow.status();
    ...
    private void labels() {
        if (this.shouldSaveUpdateLabels || this.shouldDeleteLabels || this.shouldSaveUpdateCells) {
            for (final SpreadsheetEngineBasicChangesCache<SpreadsheetLabelName, SpreadsheetLabelMapping> nameAndMapping : this.changes.labels.values()) {
                final SpreadsheetLabelName labelName = nameAndMapping.reference;

                final SpreadsheetEngineBasicChangesCacheStatusLabel status = (SpreadsheetEngineBasicChangesCacheStatusLabel) nameAndMapping.status();
                switch (status) {
    ...
    private void cells() {
        if (this.shouldSaveUpdateCells || this.shouldDeleteCells || this.shouldSaveUpdateLabels || this.shouldDeleteLabels || this.shouldSaveUpdateColumns || this.shouldDeleteColumns || this.shouldSaveUpdateRows || this.shouldDeleteRows) {
            ...
            for (final SpreadsheetEngineBasicChangesCache<SpreadsheetCellReference, SpreadsheetCell> referenceAndCell : this.changes.cells.values()) {
                final SpreadsheetCellReference cell = referenceAndCell.reference;

                final SpreadsheetEngineBasicChangesCacheStatusCell status = (SpreadsheetEngineBasicChangesCacheStatusCell) referenceAndCell.status();
                switch (status) {
                    case LOADED_REFERENCES_REFRESHED:
                    case SAVED_REFERENCES_REFRESHED:
                    case REFERENCE_SAVED_REFERENCES_REFRESHED:
    ...
    private void addLabelMappingCells(final SpreadsheetLabelMapping labelMapping) {
        if (this.shouldSaveUpdateCells) {
            ...
            if (null != reference && reference.isCell()) {
                ...
                final SpreadsheetEngineBasicChangesCache<SpreadsheetCellReference, SpreadsheetCell> cache = this.changes.cells.get(cell);
                if (null != cache) {
                    addReferences(
                        cell,
    ...
    private Map<SpreadsheetCellReference, Set<SpreadsheetExpressionReference>> extractExternalReferences() {
        ...
        for (final SpreadsheetEngineBasicChangesCache<SpreadsheetCellReference, SpreadsheetCell> cache : this.changes.cells.values()) {
            ...
            if (window.test(cell)) {
                ...
                references.addAll(
                    cellReferencesStore.findValuesById(
                        cell,
                        0, // offset
                        SpreadsheetEngineBasic.FIND_REFERENCES_COUNT// count
                    )
                );
    ...
    private final SpreadsheetEngineBasic engine;
