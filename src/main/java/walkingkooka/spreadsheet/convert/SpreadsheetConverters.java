...
public final class SpreadsheetConverters implements PublicStaticHelper {
    ...
    public static Converter<SpreadsheetConverterContext> storagePathTxtToClass() {
        return StorageConverters.storagePathTxtToClass();
    }
    ...
    public final static ConverterSelector SYSTEM_CONVERTER_SELECTOR = ConverterSelector.parse(
        "collection(text, boolean, number, date-time, basic, spreadsheet-value, error-throwing, color, expression, environment, json, locale, currency, plugins, spreadsheet-metadata, style, text-node, template, net, form-and-validation)"
    );
