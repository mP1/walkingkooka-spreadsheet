 ...
 */
public final class TextSpreadsheetParserToken extends ValueSpreadsheetParserToken {

    static TextSpreadsheetParserToken with(final List<ParserToken> value, final String text) {
        return new TextSpreadsheetParserToken(
                copyAndCheckTokens(value),
                Objects.requireNonNull(text, "text") // empty text is allowed to support a formula with empty text
        );
    }

    private TextSpreadsheetParserToken(final List<ParserToken> value,
                                       final String text) {
        super(value, text);
        ...
        for (final ParserToken token : this.children()) {
            ...
            if (spreadsheetParserToken.isTextLiteral()) {
                ...
                textValue = spreadsheetParserToken.cast(TextLiteralSpreadsheetParserToken.class)
                        .value();
            }
        }
    ...
    @Override
    public TextSpreadsheetParserToken setChildren(final List<ParserToken> children) {
