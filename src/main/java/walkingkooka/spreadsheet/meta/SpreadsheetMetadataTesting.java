...
public interface SpreadsheetMetadataTesting extends Testing {
    ...
    SpreadsheetMetadata METADATA_EN_AU = SpreadsheetMetadata.EMPTY
        ...
        .set(
            ...
            ConverterSelector.parse("collection(text, number, date-time, basic, spreadsheet-value, boolean, error-throwing, color, expression, environment, json, locale, plugins, spreadsheet-metadata, style, text-node, template, net)"
            )
        ).set(
            ...
            ValidatorAliasSet.parse("absolute-url, choice-list, collection, email-address, expression, non-null, text-length, text-mask")
        ).set(
